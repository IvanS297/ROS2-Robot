...
class ICPNode(Node):
    ...
    def icp(self, S_move, S_fix, max_iterations=20, tolerance=1e-6, max_distance=1.0, threshold_iter=20):
        ...
        for _ in range(max_iterations):
            ...
            distance = math.sqrt(math.pow(t[0], 2) + math.pow(t[1], 2))
            if distance > 1.0:
...
class ICPNode(Node):
    ...
    def voxel_grid_filter(self, points, voxel_size):
        """
        Фильтрация Voxel Grid: уменьшает плотность облака точек.
        """
        if len(points) == 0:
            return np.empty((0, 3))

        # Вычисляем индексы вокселей для каждой точки
        voxel_indices = np.floor(points / voxel_size).astype(int)

        # Группируем точки по уникальным индексам вокселей и находим центроиды
        ...
        for i in range(len(unique_indices)):
            # Берем среднее значение (центроид) всех точек в данном вокселе
            filtered_points[i] = np.mean(points[inverse_indices == i], axis=0)
