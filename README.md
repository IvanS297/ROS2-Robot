...
Системные требования (на чем сделано):
 - `Ubuntu 24.04` или `Docker образ`
 - `ROS2 Jazzy Jalisco`
 - `rclcpp` и `rclpy`
 - `Raspberry pi 4`
 - `ESP32`/`Arduino`/`STM32`
...
| :--- | :--- | :--- | :--- |
| **debug** | `Boolean` | `true` | Режим отладки. `true` — публикует топики `/frontier_cells`, `/cost_map`, `/cspace` и др. `false` — минимум топиков, меньше нагрузки на CPU. |
| **num_explore_fails_before_finish** | `Integer` | `60` | Количество отказов подряд («нет фронтиров»/«нет пути») до завершения исследования. Увеличить для медленного SLAM, уменьшить для быстрой остановки. |
| **min_map_cells** | `Integer` | `300` | Минимальный размер карты (ширина * высота клеток) перед стартом. Ожидание роста карты после запуска SLAM. |
| **min_frontier_size** | `Integer` | `6` | Минимальное количество клеток во фронтире. Меньше — для маленьких карт, больше (8-10) — для игнорирования шумов. |
| **max_frontiers_to_check** | `Integer` | `8` | Количество лучших по размеру фронтиров для проверки через A*. Увеличить для точности, уменьшить для слабых ПК. |
| **a_star_cost_weight** | `Float` | `10.0` | Вес стоимости пути A\*. Увеличить для предпочтения более ближних фронтиров. |
| **frontier_size_cost_weight** | `Float` | `1.0` | Вес размера фронтира. Увеличить для предпочтения более крупных фронтиров. |
...
| **lookahead_distance** | `Float` | `0.25` | Расстояние lookahead (м). Больше — плавнее, но срезает углы. Меньше — точнее, но дёргается. Рекомендуется 0.3-0.5 для роботов с плохими моторами. |
| **wheel_base** | `Float` | `0.19` | Расстояние между колёсами (м). Должно совпадать с URDF. |
| **max_drive_speed** | `Float` | `0.08` | Максимальная линейная скорость (м/с). Уменьшить, если моторы не успевают или робот нестабилен. |
| **max_turn_speed** | `Float` | `0.6` | Максимальная угловая скорость (рад/с). Уменьшить до 0.6-0.8 для корректной отработки поворотов обоими моторами. |
| **turn_speed_kp** | `Float` | `1.0` | Пропорциональный коэффициент угловой скорости. |
| **distance_tolerance** | `Float` | `0.01` | Допуск до цели (м). Расстояние до конечной точки пути, при котором робот останавливается. |
| **obstacle_avoidance_gain** | `Float` | `0.06` | Сила отталкивания от стен. Увеличить, если врезается. Уменьшить, если траектория сильно искажается. |
| **obstacle_avoidance_max_slow_down_distance** | `Float` | `0.05` | Расстояние (м), при котором начинается замедление робота перед препятствием. |
| **obstacle_avoidance_min_slow_down_distance** | `Float` | `0.05` | Расстояние (м), при котором замедление робота становится максимальным. |
| **obstacle_avoidance_min_slow_down_factor** | `Float` | `0.25` | Минимальный коэффициент скорости при торможении у стены. `0.25` = замедление до 25% от `max_drive_speed`. |
| **fov** | `Integer` | `300` | Угол основного поля зрения (градусы), в секторе которого учитываются стены. |
| **fov_distance** | `Integer` | `35` | Радиус основного поля зрения (измеряется в клетках карты). |
| **fov_deadzone** | `Integer` | `60` | Мёртвая зона спереди (градусы). Стены прямо перед роботом игнорируются, чтобы он мог ехать вдоль них. |
| **small_fov** | `Integer` | `300` | Малое поле зрения для близких препятствий (градусы). |
| **small_fov_distance** | `Integer` | `10` | Радиус малого поля зрения (измеряется в клетках карты). |
...
![ekf_slam](images/ekf_slam.png)

Но чтобы пользоваться IMU надо его откалибровать: есть пакет `imu_tools` в `src/`. В README описан весь процесс калибровки. Его необходимо выполнить.
...
## Список компонентов:
 ...
 - JGB37 520
 - RPLIDAR C1
 - ZyXel Keenetic Lite (роутер не обязательно)
 - 18650 3s BMS сборка
 - Web Camera
 - DS3231
 - ICM20948
 - 4 светодиода
 - Преодхранитель на 4 а
 - Защитный диод SMBJ5.0
 - USB Hub
